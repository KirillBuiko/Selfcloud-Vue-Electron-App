...
export class SocketEmitActions {
    constructor(private D: $WebRTCWorkerActions & $Socket) {}
    ...
    async getVirtualDisks(): Promise<VirtualDiskData[]>{
        ...
        return new Promise((resolve, reject) => {
            this.D.socket.timeout(5000).emit("get-virtual-disks", (err, vds) => {
                if(err){
                    reject(err);
                }
                resolve(vds);
    ...
    provideVirtualDisks(vdIDs: string[]){
        // TODO check
        this.D.socket.emit("provide-virtual-disks", vdIDs);
    }
    ...
    revokeVirtualDisk(vdID: string){
        // TODO check
        this.D.socket.emit("revoke-virtual-disk", vdID);
    }
    ...
    createVirtualDisk(): Promise<VirtualDiskData>{
        // TODO check
        return new Promise((resolve, reject) => {
            this.D.socket.timeout(5000).emit("create-virtual-disk", (err, vd) => {
                if(err){
                    reject(err);
                }
                resolve(vd);
    ...
    removeVirtualDisk(vdID: string){
        // TODO check
        this.D.socket.emit("remove-virtual-disk", vdID);
    }
    ...
    connectToDevice(targetID: string, fingerprint: string, offer: string){
        // TODO check
        this.D.socket.emit("connect-webrtc", targetID, fingerprint, offer);
    }
    ...
    acceptConnectionToDevice(targetID: string, fingerprint: string, answer: string){
        // TODO check
        this.D.socket.emit("connect-webrtc-answer", targetID, fingerprint, answer);
    }
    ...
    sendWebRTCCandidate(targetID: string, fingerprint: string, candidate: string){
        // TODO check
        this.D.socket.emit("send-webrtc-candidate", targetID, fingerprint, candidate);
    }
}
