...
export default class AccountRequestClass extends AbstractRequest{
    ...
    async loginPassword(loginData: LoginData): Promise<ResponseData<object>>{
        const response = await this.servs.requestHandler.makeRequest<RefreshData>({
            // TODO: set URL
        ...
        if(response.code == ResultCode.OK && response.result !== undefined){
            ...
            this.servs.requestHandler.setTokens(tokens);
        }
        return response;
    }
    ...
    async deleteAccount(): Promise<ResponseData<object>>{
        this.servs.requestHandler.setTokens({access: "", refresh: ""});
        return await this.servs.requestHandler.makeRequest({
            url: "",
        });
    }
